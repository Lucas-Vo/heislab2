...
const (
	netSnapshotTimeout      = 2 * time.Second
	hallRequestAssignerPath = "./elevassigner/hall_request_assigner"
)
...
func assignerThread(config common.Config, netUpdateAssignerCh <-chan common.Snapshot, elevatorTasksCh chan<- common.ElevInput) {
	...
	networkTimeout := time.NewTimer(netSnapshotTimeout)
	...
	for {
		...
		case networkSnapshot := <-netUpdateAssignerCh:
			...
			networkTimeout.Reset(netSnapshotTimeout)
			...
			ret, err := exec.Command(hallRequestAssignerPath, "-i", string(jsonBytes)).CombinedOutput()
			if err != nil {
